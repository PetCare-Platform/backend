...
3. 최종 선정한 `REDIS_KAFKA` 방식으로 재고 10,000개, 요청 20,000건 요구사항을 검증합니다.
...
- **최종 검증:** 실험 결과로 선정한 Redis/Kafka 방식만 10,000/20,000 규모로 검증합니다.
...
| 공통 최대 | 1,000 | 2,000 | 다섯 전략의 처리량·지연시간·정합성 최종 비교 |
...
| 단계 | 재고 | 요청 수 | 목적 |
| --- | ---: | ---: | --- |
| 스모크 | 10 | 20 | Kafka 발행·소비·DB 저장 흐름 확인 |
| 기본 | 100 | 200 | 동기/비동기 처리의 응답시간과 정합성 비교 |
| 중간 | 500 | 1,000 | 처리량 및 비동기 적체 변화 확인 |
| 공통 최대 | 1,000 | 2,000 | Redis 단독과 Redis/Kafka 최종 비교 |

### 8.3 최종 요구사항 검증

| 대상 | 재고 | 요청 수 | 목적 |
| --- | ---: | ---: | --- |
| `REDIS_KAFKA` | 10,000 | 20,000 | 초과 발급 0건, 1인 1매, 최종 정합성 및 대규모 처리 성능 검증 |
...
`REDIS`와 `REDIS_KAFKA`는 위 조건에 더해 다음 조건을 확인합니다.
