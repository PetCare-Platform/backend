...
DB의 Pessimistic Write Lock을 이용하여 동일한 재고에 대한 변경을 직렬화한다.
...
sequenceDiagram
    ...
    participant DB as CouponStock
    participant B as Transaction B

    A->>DB: PESSIMISTIC_WRITE
    DB-->>A: Lock 획득

    B->>DB: PESSIMISTIC_WRITE
...
조건을 포함한 UPDATE를 통해 재고 확인과 차감을 하나의 DB 연산으로 수행하는 전략을 실험한다.
...
Redis를 활용하여 재고 또는 Lock을 관리하는 전략을 실험한다.
